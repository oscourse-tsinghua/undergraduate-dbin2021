...
本文中，作者对基于GeoHash编码的地图数据存储进行了相关研究，将传统的经纬度表示的地图数据表示为基于GeoHash编码的地图数据，并将所有数据存入区块链上进行存储，去除对外部地图数据的依赖，完全使用区块链上的数据，使得地图数据安全性得到保障。树状区块链是为了区块链结构更适应车载自组网而对底层区块链结构进行的更改，增添了位置信息，实现区块链与地理信息进行绑定
