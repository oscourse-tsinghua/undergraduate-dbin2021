...
contract StoreMap{
	...
	*/
	...
	{	
		...
		bytes32 name;   //道路名称 
		uint path_num;	
		mapping(uint => bytes32) path; // 道路数组
	}
	...
	function get_road(uint gid) view public returns (int32[] memory road, bytes32 names, bytes32[] memory path) {
		road = new int32[](7);
		road_info storage single_road = ways[gid];
		path = new bytes32[](single_road.path_num);
	...
	function get_roads(bytes32 hash) view public returns (int32[] memory road, bytes32[] memory names, bytes32[] memory path) {
		...
		if(num > 0){
			...
			for(i=0; i< num; i++){
				...
				path[pos++] = bytes32(single_road2.path_num);
	...
	// 添加一条道路
	function add_road(uint gid, int32 x1, int32 y1, int32 x2, int32 y2, int32 source, int32 target, bool oneway, bytes32 name, bytes32[] memory path) public restricted {
	    ways[gid].x1 = x1;
	    ways[gid].x2 = x2;	
		ways[gid].y1 = y1;
		ways[gid].y2 = y2;
